...
const FileUploader = () => {
  ...
  const renderMedia = () => {
    if (fileType && uploadedFile) {
      if (fileType.startsWith('image/')) {
        return <img src={uploadedFile} alt="Uploaded File" style={{ maxWidth: '100%', maxHeight: '300px' }} />;
  ...
  return (
    ...
    >
      <input {...getInputProps()} />
      {renderMedia() || (isDragActive ? <p>Drop the files here...</p> : <p>Drag 'n' drop image or video files here, or click to select files</p>)}
    </div>
  );
};
