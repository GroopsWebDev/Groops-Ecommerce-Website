  ...
  const Upload = () => {
    const handle = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (files) {
        ...
        const maxVideoFileSize = 50 * 1024 * 1024; // 50MB in bytes
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (!allowedFormats.includes(file.type)) {
            errorMessage =
              "File format not supported. You must use .png,.jpg,.mp4,.mov,.wmv";
            break;
          }
          ...
          if (file.type.includes("image")) {
            ...
            if (file.size > maxVideoFileSize) {
              errorMessage = "Video file size must be less than 50MB.";
              break;
            }
          }
        ...
        // Add new files to the list if they passed validation
        ...
        <div className="flex flex-col place-items-center gap-3">
          ...
          <label htmlFor="upload-photo">
            <input
              ...
              multiple
              onChange={handle}
              accept=".png,.jpg,.jpeg,.Mp4,.Mov,.Wmv,.mkv"
            />
